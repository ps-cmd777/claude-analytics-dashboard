...
def session(tmp_path):
    """Create a test session and return the Session object (has session_id + session_token)."""
    ...
    return create_session("sales.csv", df, profiler, profile)
...
def test_chat_streams_tokens(client, session):
...
def test_chat_appends_to_history(client, session):
    ...
    assert s.chat_history[0]["content"] == "Which region has the highest sales?"
    assert s.chat_history[1]["role"] == "assistant"
    assert "East" in s.chat_history[1]["content"]


def test_chat_multiple_turns_accumulate_history(client, session):
    ...
    with patch("services.claude_client.anthropic.Anthropic", return_value=mock_client):
        with client.stream(
            "POST", f"/api/chat/{session.session_id}", json={"message": "Question 1"}, cookies=cookies,
        ) as r:
            r.read()

        with client.stream(
            "POST", f"/api/chat/{session.session_id}", json={"message": "Question 2"}, cookies=cookies,
        ) as r:
            r.read()

    s = get_session(session.session_id)
    assert s is not None
    assert len(s.chat_history) == 4  # 2 user + 2 assistant


def test_chat_empty_message_rejected(client, session):
...
def test_chat_done_event_has_no_result(client, session):
